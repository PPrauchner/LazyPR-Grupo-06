...
Cada página em viewss/` deve seguir o padrão: **ler do `st.session_state` → aplicar filtros → chamar aggregations → chamar charts → renderizar**. Nenhuma página deve acessar o dataset bruto diretamente.
...
## 17. 🧠 Instruções para o Modelo (Obrigatórias)

Você está atuando como um desenvolvedor dentro deste projeto.

Antes de responder qualquer solicitação:

1. Identifique claramente em qual camada o problema está:
   - `core/` → lógica pura
   - `services/` → I/O, LLM, rede
   - `ui/` → Streamlit

2. Aplique rigorosamente as regras dessa camada.

3. Nunca viole os princípios fundamentais:
   - Programação funcional
   - Imutabilidade
   - Lazy evaluation
   - Separação entre core e efeitos colaterais

### Regras obrigatórias:

- Nunca sugerir código imperativo dentro de `core/`
- Nunca usar `for` ou `while` em transformações de dados no `core/`
- Nunca modificar estruturas de dados in-place
- Nunca misturar lógica de negócio com I/O
- Nunca acessar variáveis globais ou estado externo em funções puras

Caso uma solicitação viole essas regras:

- Explique por que é inválida
- Sugira uma alternativa compatível com a arquitetura

---

## 18. 📐 Formato de Resposta

Todas as respostas devem seguir EXATAMENTE este formato:

### 1. Explicação

- Máximo de 5 linhas
- Direta e objetiva
- Referenciando regras do projeto quando relevante

### 2. Código

- Código completo e funcional
- Com type hints
- Sem placeholders ou pseudocódigo
- Seguindo estritamente a arquitetura do projeto

### 3. Justificativa

- Explique brevemente por que a solução está correta
- Referencie explicitamente:
  - princípios funcionais
  - separação de camadas
  - imutabilidade ou lazy evaluation (quando aplicável)

---

## 19. 🚫 Anti-Exemplos (Proibidos)

### ❌ Uso imperativo em transformações (PROIBIDO)
...
### ❌ Materialização de dataset (PROIBIDO)
...
```python
(x for x in stream if predicate(x))
```

---

### ❌ Lógica no lugar errado (PROIBIDO)
...
### ✅ Correto (delegar ao core)

```python
count = core_function(records)
```
...
Seu objetivo é preservar a integridade arquitetural e funcional do sistema.

Prioridades:

1. Garantir pureza funcional no `core/`
2. Evitar qualquer regressão para código imperativo
3. Manter imutabilidade em todas as estruturas
4. Garantir lazy evaluation em pipelines de dados
5. Respeitar rigorosamente a separação de responsabilidades

Se houver conflito entre:

- simplicidade vs arquitetura → priorize arquitetura
- performance vs pureza → priorize pureza (exceto se explicitamente indicado)

---

## 21. ⚠️ Regras de Decisão

Ao gerar soluções:

- Prefira `map`, `filter`, `reduce` sobre loops
- Prefira `tuple` sobre `list`
- Prefira funções pequenas e compostas
- Prefira composição (`compose`, `pipe`) ao invés de lógica linear longa

Se múltiplas soluções forem possíveis:

- Escolha a mais alinhada com programação funcional
- Evite qualquer forma de mutação
- Evite materialização desnecessária

---

## 22. 🔍 Validação Interna (Antes de Responder)

Antes de gerar qualquer resposta, valide mentalmente:

- [ ] Está na camada correta?
- [ ] Usa apenas funções puras (se core)?
- [ ] Evita loops imperativos?
- [ ] Mantém imutabilidade?
- [ ] Mantém lazy evaluation?
- [ ] Segue o formato de resposta exigido?
...
## 23. 🧪 Tratamento de Ambiguidade

Se uma solicitação for ambígua:

- Não assuma comportamento implícito
- Declare a suposição feita
- Escolha a alternativa mais alinhada com:
  - programação funcional
  - imutabilidade
  - arquitetura definida

Exemplo:
"Assumindo que esta função pertence ao core/, aplicarei regras de pureza funcional."

---

## 24. 🔐 Restrições de Segurança e Escopo

- Nunca acessar arquivos fora do contexto do projeto
- Nunca sugerir execução de código externo sem necessidade
- Nunca expor ou manipular variáveis de ambiente sensíveis
- Nunca alterar estrutura de pastas sem justificativa explícita
...
👉 Sempre siga este documento como fonte de verdade absoluta.

Nenhuma resposta deve contradizer estas instruções.
