...
def group_by(key_fn: Callable[[Any], Any], records: Iterable[Any]) -> Dict[Any, Tuple[Any, ...]]:
    """
...
def group_by_language_and_clarity(records: Iterable[Any]) -> Dict[Tuple[str, str], Tuple[Any, ...]]:
    """
    Groups records simultaneously by multiple dimensions: language and clarity level.
    """
    return group_by(
        lambda r: (getattr(r, 'language', 'unknown'), getattr(r, 'clarity_level', 'unknown')),
        records
    )


@lru_cache(maxsize=1)
def cross_count_language_clarity(records: Tuple[Any, ...]) -> Dict[Tuple[str, str], int]:
    """
